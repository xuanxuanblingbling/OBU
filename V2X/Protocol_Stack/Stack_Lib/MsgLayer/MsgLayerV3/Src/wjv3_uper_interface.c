...
/* 函数调度关系表 */
FUNC_WJV3_UPER_SCHE g_pfuncMsgSche[WJV3_MessageFrame_PR_TOTALNUM] = {
    /* BasicSafetyMessage */
    {WJV3_MessageFrame_PR_bsmFrame_NO, WJV3_BasicSafetyMessage_MsgEnc, WJV3_BasicSafetyMessage_MsgDec},
    /* MapData */
    {WJV3_MessageFrame_PR_mapFrame_NO, WJV3_MapData_MsgEnc, WJV3_MapData_MsgDec},
    /* RoadsideSafetyMessage */
    {WJV3_MessageFrame_PR_rsmFrame_NO, WJV3_RoadsideSafetyMessage_MsgEnc, WJV3_RoadsideSafetyMessage_MsgDec},
    /* SPAT */
    {WJV3_MessageFrame_PR_spatFrame_NO, WJV3_SPAT_MsgEnc, WJV3_SPAT_MsgDec},
    /* RoadSideInformation */
    {WJV3_MessageFrame_PR_rsiFrame_NO, WJV3_RoadSideInformation_MsgEnc, WJV3_RoadSideInformation_MsgDec}};

static int (*pFunGetAidCompleteV3)(void *pstruMsg);
__attribute__((visibility("default"))) void SetGetAIDFromFileCallbackV3(int (*pFunGetAid)(void *pstruMsg))
{
    pFunGetAidCompleteV3 = pFunGetAid;
}
...
/**
 * @brief     WJV3_CheckMsgCount检查消息号函数
...
* </table>
*/
void WJV3_CheckMsgCount(MessageFrame_t *pstruMsgFrame, int CertSwitchState)
{
    /* 入参为空检查 */
    ...
    case MessageFrame_PR_rsiFrame:
        ...
        //时间类动态信息调整
        if (NULL != pstruMsgFrame->choice.rsiFrame.timeStamp)
            *(pstruMsgFrame->choice.rsiFrame.timeStamp) = FL_Timestamp_now(5);
        break;
    default:
        LogPrint(LOG_LEVEL_ERR,
                 "MsgLayer",
...
/**
 * @brief     WJV3_UperEncodeMain消息层编码主函数
...
* </table>
 */
DLL_PUBLIC int32_t WJV3_UperEncOrDecMain(WJV3_UperData *pstruUperData, WJV3_STRU_MessageFrame *pstruWjMsg, WJV3_ENUM_InterFace InterFace)
{
    /* 入参为空检查 */
    ...
    if (InterFace == MSGV3_MessageFrame_UperDecode)
    {
        /* 变量定义 */
        ...
        /* 结构体映射,遍历数组，查询函数 */
        for (u32i = 0; u32i < WJV3_MessageFrame_PR_TOTALNUM; u32i++)
        {
        ...
        /* 超过数组长度 */
        if (u32i >= WJV3_MessageFrame_PR_TOTALNUM)
        {
            LogPrint(LOG_LEVEL_ERR,
                     STACK_MSG_MODULE,
    ...
    else if (InterFace == MSGV3_MessageFrame_UperEncode)
    {
        /* 变量定义 */
            ...
            if (pstruWjMsg->enumMessageFrameType == g_pfuncMsgSche[u32i].u32MsgNo)
            {
                ...
                else
                {
                    ...
                    struAsnEncRet = uper_encode_to_buffer(&asn_DEF_MessageFrame,
                                                          ...
                                                          (size_t)MAXV3_UPERBUFF_SIZE); /* 码流的最大长度 */
                    if (NULL != pstruAsnMsgFrame)
                    {
                        ASN_STRUCT_FREE(asn_DEF_MessageFrame, pstruAsnMsgFrame);
                    }
...
/**
 * @brief     WJV3_UperDecodeMain消息层文件读取编码主函数
...
* </table>
*/
DLL_PUBLIC int32_t WJV3_UperFromFileEncodeMain(WJV3_UperData *pstruUperData, char *pn8FileName, int *pn32MsgAid)
{
    /* 入参为空检查 */
    ...
    // printf("struAsnDecRet.consumed = %d\n", struAsnDecRet.consumed / 8);
    WJV3_CheckMsgCount(pstruMsgFrame, 0);
    if (NULL != pFunGetAidCompleteV3)
    {
        *pn32MsgAid = pFunGetAidCompleteV3(pstruMsgFrame);
    }
    /* 调用UPER_encode进行编码 */
    struAsnEncRet = uper_encode_to_buffer(&asn_DEF_MessageFrame,
                                          0,
