...
/* 函数调度关系表 */
FUNC_WJVN4_UPER_SCHE g_pfuncMsgSche[WJVN4_MessageFrame_PR_TOTALNUM] = {
    /* BasicSafetyMessage */
    {WJVN4_MessageFrame_PR_bsmFrame_NO, WJVN4_BasicSafetyMessage_MsgEnc, WJVN4_BasicSafetyMessage_MsgDec},
    {WJVN4_MessageFrame_PR_bsmFrameOC_NO, WJVN4_BasicSafetyMessage_MsgEnc, WJVN4_BasicSafetyMessage_MsgDec},
    {WJVN4_MessageFrame_PR_bsmFrameOE_NO, WJVN4_BasicSafetyMessage_MsgEnc, WJVN4_BasicSafetyMessage_MsgDec},
    {WJVN4_MessageFrame_PR_bsmFrameSC_NO, WJVN4_BasicSafetyMessage_MsgEnc, WJVN4_BasicSafetyMessage_MsgDec},
    {WJVN4_MessageFrame_PR_bsmFrameSE_NO, WJVN4_BasicSafetyMessage_MsgEnc, WJVN4_BasicSafetyMessage_MsgDec},
    /* MapData */
    {WJVN4_MessageFrame_PR_mapFrame_NO, WJVN4_MapData_MsgEnc, WJVN4_MapData_MsgDec},
    /* RoadsideSafetyMessage */
    {WJVN4_MessageFrame_PR_rsmFrame_NO, WJVN4_RoadsideSafetyMessage_MsgEnc, WJVN4_RoadsideSafetyMessage_MsgDec},
    /* SPAT */
    {WJVN4_MessageFrame_PR_spatFrame_NO, WJVN4_SPAT_MsgEnc, WJVN4_SPAT_MsgDec},
    /* RoadSideInformation */
    {WJVN4_MessageFrame_PR_rsiFrame_NO, WJVN4_RoadSideInformation_MsgEnc, WJVN4_RoadSideInformation_MsgDec},
    {WJVN4_MessageFrame_PR_rsiFrameRC_NO, WJVN4_RoadSideInformation_MsgEnc, WJVN4_RoadSideInformation_MsgDec},
    {WJVN4_MessageFrame_PR_rsiFrameRH_NO, WJVN4_RoadSideInformation_MsgEnc, WJVN4_RoadSideInformation_MsgDec},
    {WJVN4_MessageFrame_PR_rsiFrameRD_NO, WJVN4_RoadSideInformation_MsgEnc, WJVN4_RoadSideInformation_MsgDec},
};

static int (*pFunGetAidCompleteVN4)(void *pstruMsg);
__attribute__((visibility("default"))) void SetGetAIDFromFileCallbackVN4(int (*pFunGetAid)(void *pstruMsg))
{
    pFunGetAidCompleteVN4 = pFunGetAid;
}
...
/**
 * @brief     WJVN4_CheckMsgCount检查消息号函数
...
* </table>
*/
void WJVN4_CheckMsgCount(MessageFrame_t *pstruMsgFrame, int CertSwitchState)
{
    /* 入参为空检查 */
...
/**
 * @brief     WJVN4_UperEncodeMain消息层编码主函数
...
* </table>
 */
DLL_PUBLIC int32_t WJVN4_UperEncOrDecMain(WJVN4_UperData *pstruUperData, WJVN4_STRU_MessageFrame *pstruWjMsg, WJVN4_ENUM_InterFace InterFace)
{
    /* 入参为空检查 */
    ...
    if (InterFace == MSGVN4_MessageFrame_UperDecode)
    {
        /* 变量定义 */
        ...
        // 保留一致性测试日志，一致性测试使用，其他不使用
        // {
        //     char buf[409600] = {0};
        //     FILE *fp = fopen("./log.txt", "w+");
        //     xer_fprint(fp, &asn_DEF_MessageFrame, pstruAsnMsgFrame);
        //     fseek(fp, 0, SEEK_SET);
        //     fread(buf, 1, 409600, fp);
        //     LogPrint(LOG_LEVEL_ERR, STACK_MSG_MODULE, "一致性测试接收的消息数据 : \n %s", buf);
        //     fclose(fp);
        // }
        /* 结构体映射,遍历数组，查询函数 */
        for (u32i = 0; u32i < WJVN4_MessageFrame_PR_TOTALNUM; u32i++)
        {
        ...
        /* 超过数组长度 */
        if (u32i >= WJVN4_MessageFrame_PR_TOTALNUM)
        {
            LogPrint(LOG_LEVEL_ERR,
                     STACK_MSG_MODULE,
    ...
    else if (InterFace == MSGVN4_MessageFrame_UperEncode)
    {
        /* 变量定义 */
        ...
        for (u32i = 0; u32i < WJVN4_MessageFrame_PR_TOTALNUM; u32i++)
        {
            if (pstruWjMsg->enumMessageFrameType == g_pfuncMsgSche[u32i].u32MsgNo)
            {
                ...
                else
                {
                    ...
                    //     fseek(fp, 0, SEEK_SET);
                    //     fread(buf, 1, 409600, fp);
                    //     LogPrint(LOG_LEVEL_ERR, STACK_MSG_MODULE, "一致性测试发送的消息数据 : \n %s", buf);
                    //     fclose(fp);
                    // }
                    /* 调用UPER_encode进行编码 */
                    struAsnEncRet = uper_encode_to_buffer(&asn_DEF_MessageFrame,
                                                          ...
                                                          (size_t)MAXVN4_UPERBUFF_SIZE); /* 码流的最大长度 */
                    if (NULL != pstruAsnMsgFrame)
                    {
                        ASN_STRUCT_FREE(asn_DEF_MessageFrame, pstruAsnMsgFrame);
                    }
...
/**
 * @brief     WJVN4_UperDecodeMain消息层文件读取编码主函数
...
* </table>
*/
DLL_PUBLIC int32_t WJVN4_UperFromFileEncodeMain(WJVN4_UperData *pstruUperData, char *pn8FileName, int *pn32MsgAid)
{
    /* 入参为空检查 */
    ...
    WJVN4_CheckMsgCount(pstruMsgFrame, 0);
    if (NULL != pFunGetAidCompleteVN4)
    {
        *pn32MsgAid = pFunGetAidCompleteVN4(pstruMsgFrame);
    }
    /* 调用UPER_encode进行编码 */
    struAsnEncRet = uper_encode_to_buffer(&asn_DEF_MessageFrame,
                                          0,
