...
***********************************************************************/
int32_t FT_ClockTimeRegister(char *p_pn8RegisterName, int p_n32RegisterLen, int p_n32RegisterClockTime, sem_t **p_pptSemId)
{
    /* 入参检查 */
    ...
    /* 打开创建信号量 */
    *p_pptSemId = sem_open((char *)l_tClockTimeMsg.m_na8SemName, O_CREAT, 0666, 0);
    ...
    {
        /* 清空消息队列并重新创建 */
        l_n32Ret = msgctl(l_n32Msqid, IPC_RMID, NULL);
        if (l_n32Ret < 0)
        {
            LogPrint(LOG_LEVEL_ERR, CLOCKTIMEMODULE, "rm msg is err: l_n32Ret = %d", l_n32Ret);
            return false;
        }
        l_n32Msqid = msgget(l_tKey_Id, IPC_CREAT | 0666);
        if (l_n32Msqid < 0)
...
void Thread_ClockTime_Deal(void *arg)
{
    ...
    key_t l_tKey_Id = {0};
    int32_t l_s32CorrectValue = 0;
    int8_t l_u8Count = 0;
    int32_t l_n32Num = 0;
    uint8_t l_u8CheckFlag = 0;
    /* 创建消息队列 */
    umask(0);
    ...
    if (NULL == l_ptclocklinklist)
    {
        LogPrint(LOG_LEVEL_ERR, CLOCKTIMEMODULE, "malloc is err");
        return;
    }
    ...
    while (1)
    {
        ...
        while (0 <= l_n32Ret)
        {
            ...
            {
                /* 打开创建信号量 */
                l_ptSemId = sem_open((char *)l_tRecvClockMsg.m_na8SemName, O_CREAT, 0666, 0);
                ...
                {
                    sem_close(l_ptSemId);
                    sem_unlink((char *)l_tRecvClockMsg.m_na8SemName);
                    /* 删除链表对应的信号量 */
                    l_ptclockLinklistDel = l_ptclocklinklist->m_tnext;
                    l_ptclockLinklistDelPro = l_ptclocklinklist;
                    while (NULL != l_ptclockLinklistDel)
...
int32_t FT_ClockTimeInit(void)
{
    ...
    l_pthreadparam.sched_priority = 96;
