...
typedef enum
{
    MSGV2_MessageFrame_UperDecode = 1, /* 消息解码 */
    MSGV2_MessageFrame_UperEncode = 2, /* 消息编码 */

} WJV2_ENUM_InterFace;
/**
 * @brief SetGetAIDFromFileCallbackV2 设置获取AID回调函数接口（外部不使用）
 *
 ...
 *
 */
extern DLL_PUBLIC void SetGetAIDFromFileCallbackV2(int (*pFunGetAid)(void *pstruMsg));

/**
 * @brief WJV2_UperFromFileEncodeMain 从本地文件解析xml数据，并输出uper码流
 *
...
/**
 * @brief WJV2_UperEncOrDecMain 二期消息编解码接口
 *
 ...
 *
 */
extern DLL_PUBLIC int32_t WJV2_UperEncOrDecMain(WJV2_UperData *pstruUperData, WJV2_STRU_MessageFrame *pstruWjMsg, WJV2_ENUM_InterFace InterFace);

/*二期消息集接口*/
typedef int32_t (*WJV2_UperFromFileEncodeMain_BACK)(WJV2_UperData *pstruUperData, char *pn8FileName, int *pn32MsgAid);
typedef int32_t (*WJV2_UperEncOrDecMain_BACK)(WJV2_UperData *pstruUperData, WJV2_STRU_MessageFrame *pstruWjMsg, WJV2_ENUM_InterFace InterFace);
typedef void (*WJV2_SetGetAIDFromFileCallback)(int (*pFunGetAid)(void *pstruMsg));
