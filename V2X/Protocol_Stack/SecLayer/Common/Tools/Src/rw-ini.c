...
#include "rw-ini.h"
// LPSTR g_pSecData = NULL;    // xinchuang.xing
char g_pSecData[10240] = {'\0'};
/************************************************************************
**º¯Êý£ºCopyFile
...
**±ž×¢£º
************************************************************************/
void SecWriteValue(INT fdOutput, LPCSTR pAppName, LPCSTR pKeyName, LPCSTR pString);

static BOOL SecCopyFile(LPCSTR lpExistingFileName, // name of an existing file
...
**±ž×¢£º
************************************************************************/
static INT SecGetLine(LPSTR pLine, DWORD dwOffset, DWORD dwSize)
{
    int len = 0;
    int len2 = 0;
    // Look for the end of the line.
    while (dwOffset + len < dwSize && '\r' != g_pSecData[dwOffset + len] && '\n' != g_pSecData[dwOffset + len])
    {
        if (g_pSecData[dwOffset + len] == 0)
            break;
        pLine[len] = g_pSecData[dwOffset + len];
        ++len;
    }
    ...
    if (dwOffset + len + len2 < dwSize && '\r' == g_pSecData[dwOffset + len + len2])
        ++len2;
    if (dwOffset + len + len2 + 1 < dwSize && '\n' == g_pSecData[dwOffset + len + len2])
        ++len2;
...
static BOOL SecIsComment(LPCSTR pLine)
{
...
**返回：
		...
		0 - 不是
************************************************************************/
static BOOL SecIsSection(LPCSTR pLine)
{
    if (pLine && '[' == *pLine)
        return TRUE;
    return FALSE;
...
**±ž×¢£º
************************************************************************/
static BOOL SecIsSectionName(LPCSTR pLine, LPCSTR pSection)
{
    if (SecIsSection(pLine))
    {
...
**备注：
************************************************************************/
static BOOL SecIsKey(LPSTR pLine, LPCSTR pKeyName, LPSTR *pValue, DWORD *dwValLen)
{
    LPSTR pEqual = NULL;
...
************************************************************************/
static INT SecReadIniFile(LPCSTR lpFileName)
{
    int nReturn = 0;
    struct stat stat;
    int fdFile = 0;
    ...
    // Next, load the data.
    //printf("SecReadIniFile, the size of ini file [%s] is [%d]\n", lpFileName, dwSize);
    if (0 < dwSize)
    {
        pData = (char *)malloc((dwSize + sizeof(char)));
        if (!pData)
        {
            printf("SecReadIniFile, no enough memory\n");
            nReturn = 0;
            goto exit;
        }
        ...
        if (bRet != dwSize)
        {
            printf("SecReadIniFile, ReadFile fail, err = %s\n", strerror(errno));
            nReturn = 0;
            goto exit;
        }
        memcpy(&g_pSecData, pData, dwSize);
...
exit:
    if (pData)
    { //printf("SecReadIniFile free\n");
        free(pData);
        pData = NULL;
    }
    if (fdFile)
    {
        //printf("SecReadIniFile close\n");
        close(fdFile);
        fdFile = 0;
    }
    //printf("exit SecReadIniFile\n");
    return nReturn;
}
...
**备注：
************************************************************************/
static DWORD SecGetString(LPCSTR lpAppName, LPCSTR lpKeyName,
                          LPSTR lpReturnedString, DWORD nSize, LPCSTR lpFileName)
{
    ...
    if (0 == (dwSize = SecReadIniFile(lpFileName)))
    {
        return 0;
    }
    cchCopied = 0;
    while (0 != (dwOffset = SecGetLine(pLine, dwOffset, dwSize)))
    {
        //RETAILMSG(1,(_T("%s\n"),szLine));
        // ÊÇ²»ÊÇ×¢ÊÍÐÐ
        if (SecIsComment(pLine))
        ...
        {
            // ÊÇ²»ÊÇÎÒÃÇÒªÕÒµÄ¶ÎÃû
            if (SecIsSectionName(pLine, lpAppName))
            {
                // Ñ°ÕÒÎÒÃÇÒªµÄŒüÃû
                while (0 != (dwOffset = SecGetLine(pLine, dwOffset, dwSize)))
                {
                    ...
                    if (SecIsKey(pLine, lpKeyName, &pValue, &dwValLen))
                    {
    ...
    LPCSTR lpFileName)
{
    ...
    dwRc = SecGetString(lpAppName, lpKeyName, lpReturnedString, Size, lpFileName);
    if (dwRc != 0)
    {
        dwReturn = dwRc;
    }
    ...
    LPCSTR lpFileName)
{
    ...
    cch = SecGetString(lpAppName, lpKeyName, szRet, sizeof(szRet) / sizeof(char), lpFileName);
...
************************************************************************/
static void SecWriteLine(INT hOutput, LPCSTR pLine)
{
    if (pLine)
    {
...
************************************************************************/
static BOOL SecWritePrivateProfileString(
    LPCSTR lpAppName,
    LPCSTR lpKeyName,
    LPCSTR lpString,
    LPCSTR lpFileName)
{
    ...
    dwSize = SecReadIniFile(lpFileName);
    ...
    for (;;)
    {
        ...
        if (bReadLine)
        {
            dwOffset = SecGetLine(pszLine, dwOffset, dwSize);
            if (!dwOffset)
            {
                //printf("dwOffset = %d \n", dwOffset);
                break;
        ...
        bReadLine = TRUE;
        // Skip past comments.
        if (SecIsComment(pszLine))
        {
            SecWriteLine(fdOutputFile, pszLine);
            continue;
        }
        // Found a section name.
        if (SecIsSection(pszLine))
        {
            // It's the section we want.
            if (SecIsSectionName(pszLine, lpAppName))
            {
                ...
                while (0 != (dwOffset = SecGetLine(pszLine, dwOffset, dwSize)))
                {
                    ...
                    else if (SecIsComment(pszLine))
                    {
                        SecWriteLine(fdOutputFile, pszLine);
                        continue;
                    }
                    // Got the value we want.
                    if (!bWrote && SecIsKey(pszLine, lpKeyName, &pValue, &dwValLen))
                    {
                        bWrote = TRUE;
                        // Èç¹ûlpStringÎªNULL£¬ÉŸ³ýœ¡lpKeyName
                        if (lpString)
                            SecWriteValue(fdOutputFile, NULL, lpKeyName, lpString);
                    }
                    else
                    {
                        if (lpKeyName)
    ...
    if (!bWrote && lpKeyName && lpString)
    {
        ...
        SecWriteValue(fdOutputFile, lpAppName, lpKeyName, lpString);
    }
    ...
    if (-1 != fdOutputFile)
    {
        ...
        close(fdOutputFile);
        SecCopyFile(pszIniFileTemp, lpFileName, FALSE);
    ...
    LPCSTR lpFileName)
{
    ...
    return (SecWritePrivateProfileString(lpAppName, lpKeyName, ValBuf, lpFileName));
}
...
**²ÎÊý£º
		...
		in which data is written. If this parameter is NULL, the SecWriteValue
...
by xinchuang.xing
*/
int SecReadConfigData(LPCSTR ConfigAppName, LPCSTR ConfigKeyName, LPSTR ConfigReturnedString, DWORD ConfigSize, LPCSTR ConfigFileName)
{
    int nRet = 0;
    nRet = SecGetString(ConfigAppName, ConfigKeyName, ConfigReturnedString, ConfigSize, ConfigFileName);
    if (nRet == FALSE)
    {
...
by xinchuang.xing
*/
int SecWriteConfigData(LPCSTR ConfigAppName, LPCSTR ConfigKeyName, LPSTR ConfigValue, LPCSTR ConfigFileName)
{
    BOOL nRet = 0;
    nRet = SecWritePrivateProfileString(ConfigAppName, ConfigKeyName, ConfigValue, ConfigFileName);
    if (nRet == FALSE)
    {
