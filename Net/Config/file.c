 ...
 *
 ***********************************************************************/
int ConfigGetKey(const char *CFG_file, const char *section, const char *key,
		char *buf)
{
	FILE *fp;
	...
	while (1) /* 搜找key */
	{
		...
		if (SplitKeyValue(buf1, &key_ptr, &val_ptr) != 1)
			continue; //读到的值不是键值对    //goto r_cfg_end;
 ...
 *
 ***********************************************************************/
int ConfigSetKey(const char *CFG_file, const char *section, const char *key,
		char *buf)
{
	FILE *fp1, *fp2;
