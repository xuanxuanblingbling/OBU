...
/**
 * @Construct a MD5 object with a uint8_t array.
 *
 * @param the {data} will be transformed.
 *
 */
MD5::MD5(const uint8_t* data,const int length) {
  finished = false;
  /* Reset number of bits. */
  count[0] = count[1] = 0;
  /* Initialization constants. */
  ...
  /* Initialization the object according to data. */
  init(data, length);
}
