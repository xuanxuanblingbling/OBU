...
/***********************************************************************
 * 函数名称： GetPrivateProfileSectionNames
 * 功能描述： 写配置文件
 * 输入参数： lpszReturnBuffer    -字段名
 *          lpFileName          -配置文件名
 * 输出参数： 无
 * 返回值:    0/-1              成功/失败
 * 开发人     版本  修改人    描述
 * ========== ======= ========= =======================================
 * 2021-06-10 V1.0    xingjian.li   Create
***********************************************************************/
int GetPrivateProfileSectionNames(char lpszReturnBuffer[][50], LPCSTR lpFileName)
{
    DWORD dwSize;
    DWORD dwOffset = 0;
    char pLine[MAX_PATH] = {0};
   ...
   //打开文件，将文件数据拷贝到缓冲区g_pData中，返回INI文件大小
    ...
    while ( 0 != (dwOffset = GetLine( pLine , dwOffset , dwSize )))
    {
        ...
        if (IsSection(pLine))
        {
            strncpy(lpszReturnBuffer[mCount], pLine+1, strlen(pLine)-2);
            mCount++;
        }
    }
