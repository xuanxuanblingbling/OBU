...
int SW_versionHASHSUM(void)
{
	...
	//TSP
	cJSON *pTSPVersion = cJSON_GetObjectItem(pAppSoftware,TSP);
	if(NULL == pTSPVersion)
	{
        ret = ERR_MOUDLENAME;
		goto ERR_EXIT;	
	}
    memcpy(tmpstring,&pTSPVersion->valuestring[DatePos],6);
    SUM += atoi(tmpstring);
	memcpy(tmpstring,&pTSPVersion->valuestring[11],8);
	SoftWareDate[3] = atoi(tmpstring);
	printf("=== [test] TSP Date is : %d  \n :==\n",SoftWareDate[3]);     
	memcpy(&hashstring[lc],&pTSPVersion->valuestring[0],versionlen);
	lc += versionlen;
